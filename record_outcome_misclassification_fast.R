...
###############################################################################
# Probabilistic bias analysis for outcome misclassification: record-level data
#
# Author: R MACLEHOSE
...
#   - always uses individual-level imputation and regression, including when
#     covariates = NULL; no no-covariate 2x2-table shortcut is used
#   - precomputes the model matrix and calls glm.fit() directly
...
# -----------------------------------------------------------------------------
# Optimized individual-level regression and parallel helpers
# -----------------------------------------------------------------------------
...
build_effect_matrix <- function(data, covariate_terms = character()) {
  formula <- stats::reformulate(c(".pba_x", covariate_terms))
...
run_outcome_pba_chunk <- function(indices, worker) {
  ...
  for (k in seq_along(indices)) {
    ...
    if (isTRUE(worker$show_progress) &&
      ...
      cat("\rOutcome-misclassification draws: ", k, "/", n_local, sep = "")
      flush.console()
    }

    systematic_probabilities <- c(
      worker$ppv_exposed_expected[i],
      1 - worker$npv_exposed_expected[i],
      worker$ppv_unexposed_expected[i],
      1 - worker$npv_unexposed_expected[i]
    )
    ...
    imputation_probabilities <- c(
      worker$ppv_exposed[i],
      1 - worker$npv_exposed[i],
      worker$ppv_unexposed[i],
      1 - worker$npv_unexposed[i]
    )
    ...
    imputed_y <- as.integer(stats::runif(worker$n_records) < p_impute)
...
# -----------------------------------------------------------------------------
# Main record-level outcome-misclassification function
# -----------------------------------------------------------------------------
# se1/sp1 refer to classification among exposed persons; se0/sp0 among
# unexposed persons.

pba.record.out <- function(data,
                           exposure,
                           outcome,
                           se1.a, se1.b,
                           se0.a, se0.b,
  ...
  glm_controls <- validate_glm_controls(glm_maxit, glm_epsilon)
  glm_maxit <- glm_controls$glm_maxit
  glm_epsilon <- glm_controls$glm_epsilon
  validate_seed(seed)
  ...
  misclass <- parse_misclassification(type, "outcome")
  ...
  if (misclass$type == "nondiff") {
    ...
    se_exposed <- se_pair$first
    se_unexposed <- se_pair$second
    sp_exposed <- sp_pair$first
    sp_unexposed <- sp_pair$second
  }

  bias_draws_all <- tibble(
    draw = draw_id,
    se_exposed, se_unexposed, sp_exposed, sp_unexposed
  )

  corrected_cases_exp <-
    (a - n_exposed * (1 - sp_exposed)) / (se_exposed + sp_exposed - 1)
  corrected_noncases_exp <- n_exposed - corrected_cases_exp
  corrected_cases_unexp <-
    (b - n_unexposed * (1 - sp_unexposed)) / (se_unexposed + sp_unexposed - 1)
  corrected_noncases_unexp <- n_unexposed - corrected_cases_unexp

  valid_corrected <-
    corrected_cases_exp > 0 & corrected_noncases_exp > 0 &
    corrected_cases_unexp > 0 & corrected_noncases_unexp > 0 &
    is.finite(corrected_cases_exp) & is.finite(corrected_noncases_exp) &
    is.finite(corrected_cases_unexp) & is.finite(corrected_noncases_unexp)
  ...
  prev_exposed_expected <- corrected_cases_exp / n_exposed
  prev_unexposed_expected <- corrected_cases_unexp / n_unexposed
  ppv_exposed_expected <-
    se_exposed * prev_exposed_expected /
    (se_exposed * prev_exposed_expected +
       (1 - sp_exposed) * (1 - prev_exposed_expected))
  npv_exposed_expected <-
    sp_exposed * (1 - prev_exposed_expected) /
    ((1 - se_exposed) * prev_exposed_expected +
       sp_exposed * (1 - prev_exposed_expected))
  ppv_unexposed_expected <-
    se_unexposed * prev_unexposed_expected /
    (se_unexposed * prev_unexposed_expected +
       (1 - sp_unexposed) * (1 - prev_unexposed_expected))
  npv_unexposed_expected <-
    sp_unexposed * (1 - prev_unexposed_expected) /
    ((1 - se_unexposed) * prev_unexposed_expected +
       sp_unexposed * (1 - prev_unexposed_expected))

  n_compatible <- length(draw_id)
  prev_exposed <- stats::rbeta(
    n_compatible, corrected_cases_exp, corrected_noncases_exp
  )
  prev_unexposed <- stats::rbeta(
    n_compatible, corrected_cases_unexp, corrected_noncases_unexp
  )
  ppv_exposed <-
    se_exposed * prev_exposed /
    (se_exposed * prev_exposed + (1 - sp_exposed) * (1 - prev_exposed))
  npv_exposed <-
    sp_exposed * (1 - prev_exposed) /
    ((1 - se_exposed) * prev_exposed + sp_exposed * (1 - prev_exposed))
  ppv_unexposed <-
    se_unexposed * prev_unexposed /
    (se_unexposed * prev_unexposed +
       (1 - sp_unexposed) * (1 - prev_unexposed))
  npv_unexposed <-
    sp_unexposed * (1 - prev_unexposed) /
    ((1 - se_unexposed) * prev_unexposed +
       sp_unexposed * (1 - prev_unexposed))

  valid_predictive <-
    is.finite(ppv_exposed) & is.finite(npv_exposed) &
    is.finite(ppv_unexposed) & is.finite(npv_unexposed) &
    is.finite(ppv_exposed_expected) & is.finite(npv_exposed_expected) &
    is.finite(ppv_unexposed_expected) & is.finite(npv_unexposed_expected) &
    ppv_exposed >= 0 & ppv_exposed <= 1 &
    npv_exposed >= 0 & npv_exposed <= 1 &
    ppv_unexposed >= 0 & ppv_unexposed <= 1 &
    npv_unexposed >= 0 & npv_unexposed <= 1 &
    ppv_exposed_expected >= 0 & ppv_exposed_expected <= 1 &
    npv_exposed_expected >= 0 & npv_exposed_expected <= 1 &
    ppv_unexposed_expected >= 0 & ppv_unexposed_expected <= 1 &
    npv_unexposed_expected >= 0 & npv_unexposed_expected <= 1
  ...
  ppv_exposed_expected <- ppv_exposed_expected[valid_predictive]
  npv_exposed_expected <- npv_exposed_expected[valid_predictive]
  ppv_unexposed_expected <- ppv_unexposed_expected[valid_predictive]
  npv_unexposed_expected <- npv_unexposed_expected[valid_predictive]
  ...
  cell_id <- ifelse(
    ...
    ifelse(
      x == 1L & y_obs == 0L, 2L,
      ifelse(x == 0L & y_obs == 1L, 3L, 4L)
    )
  )

  X_base <- observed_context$X
  exposure_col <- observed_context$exposure_col
  X_systematic <- rbind(X_base, X_base)
  y_systematic <- c(rep.int(1L, n_records), rep.int(0L, n_records))
  ...
  worker_data <- list(
    ...
    cell_id = cell_id,
    ppv_exposed_expected = ppv_exposed_expected,
    npv_exposed_expected = npv_exposed_expected,
    ppv_unexposed_expected = ppv_unexposed_expected,
    npv_unexposed_expected = npv_unexposed_expected,
    ppv_exposed = ppv_exposed,
    npv_exposed = npv_exposed,
    ppv_unexposed = ppv_unexposed,
    npv_unexposed = npv_unexposed,
  ...
  parallel_run <- run_parallel_chunks(
    chunks = chunks,
    worker_fun = run_outcome_pba_chunk,
    worker_data = worker_data,
    cores_used = cores_used,
    progress = progress,
    label = "Outcome-misclassification draws"
  )
  ...
  bias_draws_valid <- tibble(
    ...
    se_exposed = se_exposed[accepted_index],
    se_unexposed = se_unexposed[accepted_index],
    sp_exposed = sp_exposed[accepted_index],
    sp_unexposed = sp_unexposed[accepted_index]
  )
  ...
  out <- list(
    ...
    analysis_label = "Record-level outcome misclassification",
    bias_label = NULL,
    model_label = model_label(effect_measure, rr_method, prep$covariates),
    exposure = exposure,
    outcome = outcome,
    ...
    computation_path = paste0(
      "individual-level outcome imputation; precomputed model matrix with ",
      "glm.fit(); ", cores_used,
  ...
  class(out) <- c("pba_record_out_sim", "pba_sim")
  out
}

# Descriptive alias.
pba.record.outcome <- pba.record.out
...
if (RUN_EXAMPLE) {
  ...
  D <- data.frame(
    ...
    d_obs = c(rep(1, a + b), rep(0, c + d))
  )
  set.seed(20260802)
  D$age <- stats::rnorm(nrow(D), mean = 50, sd = 10)

  draws.out <- pba.record.out(
    data = D,
    exposure = "exposure",
    outcome = "d_obs",
    se1.a = 254, se1.b = 24,
    se0.a = 450, se0.b = 67,
    sp1.a = 168.392, sp1.b = 5.208,
    sp0.a = 591.431, sp0.b = 47.954,
    type = "diff(.80)",
    effect_measure = "RR",
    covariates = "age",
