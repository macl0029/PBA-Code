# SCRIPT VERSION: 2026-08-02-v7-record-level-optional-covariates
###############################################################################
...
fit_effect_matrix <- function(X,
  ...
  if (need_se) {
    ...
    bread <- suppressWarnings(try(chol2inv(R), silent = TRUE))
    if (inherits(bread, "try-error") || any(!is.finite(bread))) return(NULL)

    if (effect_measure == "RR" && rr_method == "modified-poisson") {
      prior_weights <- if (is.null(weights)) rep.int(1, n) else weights
      residual <- y - fit$fitted.values
      if (frequency_weights) {
        # Frequency weights represent repeated individual records.  The meat
        # therefore contains w_i copies of the individual score, not the square
        # of a single score multiplied by w_i^2.
        meat_weight <- prior_weights * residual^2
      } else {
        meat_weight <- (prior_weights * residual)^2
      }
      X_rank <- X[, pivot, drop = FALSE]
      meat <- crossprod(X_rank, X_rank * meat_weight)
      covariance <- bread %*% meat %*% bread
    } else {
      covariance <- bread
    }

    variance <- unname(covariance[exposure_position, exposure_position])
    if (!is.finite(variance) || variance < 0) return(NULL)
    se <- sqrt(variance)
  }
...
pba.record.conf <- function(data,
  ...
  if (has_covariates) {
    ...
    # Match the random-error-only calculation in the original record-level
    # program when the fitted model contains exposure alone.
  ...
  systematic_start <- initial_start
  adjusted_start <- initial_start

  for (i in seq_len(n_candidates)) {
    if (isTRUE(progress) &&
        (i %% progress_every == 0L || i == n_candidates)) {
      cat("\rUncontrolled-confounding draws: ", i, "/", n_candidates,
          sep = "")
      flush.console()
    }

    probability_values <- c(
      pr_e1d1[i], pr_e0d1[i], pr_e1d0[i], pr_e0d0[i]
    )
    p_impute <- probability_values[cell_id]

    if (has_covariates) {
      systematic_fit <- fit_effect_matrix(
        X = X_systematic,
        y = y_systematic,
        effect_measure = effect_measure,
        rr_method = rr_method,
        exposure_col = exposure_col,
        weights = c(p_impute, 1 - p_impute),
        need_se = FALSE,
      ...
      systematic_start <- update_start(systematic_fit, systematic_start)
      syst_all[i] <- systematic_fit$effect
    }

    # Individual-level confounder imputation is used regardless of whether
    # measured covariates were supplied.
    imputed_c <- as.integer(stats::runif(nrow(base)) < p_impute)

    # Reproduce the original zero-cell exclusion using the four imputed
    # confounder-present exposure-by-outcome cells.  The optional
    # any-stratum rule also checks the four confounder-absent cells.
    c1_counts <- tabulate(cell_id[imputed_c == 1L], nbins = 4L)
    c0_counts <- c(a, b, c, d) - c1_counts
    ...
    if (zero_cell_failed[i]) next

    X_work[, confounder_col] <- imputed_c
    imputed_fit <- fit_effect_matrix(
      X = X_work,
      y = y,
      effect_measure = effect_measure,
      rr_method = rr_method,
    ...
    adjusted_start <- update_start(imputed_fit, adjusted_start)

    adjusted_all[i] <- imputed_fit$effect
    adjusted_se_all[i] <- imputed_fit$se
    accepted[i] <-
      is.finite(syst_all[i]) & is.finite(adjusted_all[i]) &
      is.finite(adjusted_se_all[i]) & adjusted_se_all[i] >= 0 &
      (effect_measure == "RD" ||
         (syst_all[i] > 0 && adjusted_all[i] > 0))
  }
  if (isTRUE(progress)) cat("\n")
  computation_path <- if (has_covariates) {
    "individual-level confounder imputation; precomputed covariate model matrix with glm.fit()"
  } else {
    "individual-level confounder imputation; exposure-plus-confounder model with glm.fit()"
  }

  # In the original program, the systematic-error distribution was calculated
  # from every bias-parameter draw and was not conditioned on whether the later
  # stochastic confounder imputation happened to produce a zero cell.  Keep
  # that distribution separate from the draws eligible for total error.
  ...
  out <- list(
    ...
    rejection_counts = tibble(
      ...
      Rejected = c(
        rejected_expected, rejected_probability,
        rejected_zero_cells, rejected_model, rejected_total
      )
    ),
...
if (RUN_EXAMPLE) {
  ...
  set.seed(20260801)
  D$age <- round(stats::rnorm(nrow(D), mean = 50, sd = 10), 1)
  ...
  # The fitted model is outcome ~ exposure + imputed confounder + age + sex.
  # For RR with rr_method="modified-poisson", the code uses an HC0 robust SE.
  # To run the same individual-level procedure without measured covariates, use:
  #   covariates = NULL
  #
  # Other zero-cell options:
  #   zero_cell_rule = "confounder-present"  # any of the four C=1 cells
  #   zero_cell_rule = "any-stratum"         # any of all eight cells
  #   zero_cell_rule = "none"
  #
