# BETA PREVALENCE VERSION v2 -- 2026-09-07
# Derived from Richard MacLehose's summary_confounding.r (2026-08-01).
# Source this file to load pba.conf.summary.mh.beta() and the original result
# table/plot helpers. Sourcing does not clear user objects, run examples, set
# a seed, or reset the current random-number stream.
#
# Confounder prevalence in exposed and unexposed groups uses independent beta
# distributions parameterized by alpha and beta. Confounder-disease RR keeps
# its independent trapezoidal distribution, including when effect_measure="OR".
# For a beta distribution mean=alpha/(alpha+beta); increasing alpha+beta at
# fixed mean makes the prior more precise. No shape rounding occurs in this code.
#
# RR is the default effect; OR and the source's RD option are also available.
# The source's MH estimates, standard errors, binomial allocation, and filtering
# are unchanged. In particular, systematic draws still use the later accepted
# allocation rows, exactly as in the supplied R source. This beta version does
# not introduce independent retention of systematic draws.
#
# Existing helper definitions are unchanged from the original R source, so
# this function can be loaded alongside its trapezoidal-prevalence counterpart.
# The examples at the end are comments; run them explicitly when desired.
...
pba.conf.summary.mh.beta <- function(
    a,
    b,
    c,
    d,
  ...
  # -----------------------------------------------------------------------
  # Validate scalar inputs before consuming any random draws.
  # -----------------------------------------------------------------------

  positive_scalar <- function(x) {
    is.numeric(x) && length(x) == 1L && is.finite(x) && x > 0
  }
  observed <- list(a = a, b = b, c = c, d = d)
  if (!all(vapply(observed, positive_scalar, logical(1))) ||
      any(vapply(observed, function(x) x != round(x), logical(1)))) {
    stop("Observed cell counts must be positive finite integers.")
  }
  if (!positive_scalar(SIMS) || SIMS != round(SIMS) ||
      SIMS > .Machine$integer.max) {
    stop("SIMS must be a positive integer no larger than .Machine$integer.max.")
  }
  shapes <- list(p1.alpha = p1.alpha, p1.beta = p1.beta,
                 p0.alpha = p0.alpha, p0.beta = p0.beta)
  if (!all(vapply(shapes, positive_scalar, logical(1)))) {
    stop("p1.alpha, p1.beta, p0.alpha and p0.beta must be positive finite scalars.")
  }
  rr_parameters <- list(rr.min, rr.mod1, rr.mod2, rr.max)
  if (!all(vapply(rr_parameters, positive_scalar, logical(1)))) {
    stop("Confounder-disease RR trapezoid parameters must be positive finite scalars.")
  }
  if (!(rr.min <= rr.mod1 && rr.mod1 <= rr.mod2 && rr.mod2 <= rr.max) ||
      !is.finite(rr.max + rr.mod2 - rr.min - rr.mod1) ||
      rr.max + rr.mod2 - rr.min - rr.mod1 <= 0) {
    stop("RR parameters must satisfy rr.min <= rr.mod1 <= rr.mod2 <= rr.max and have positive trapezoid area.")
  }
  ...
  prev_conf_exp <- stats::rbeta(niter, shape1 = p1.alpha, shape2 = p1.beta)
  prev_conf_unexp <- stats::rbeta(niter, shape1 = p0.alpha, shape2 = p0.beta)
...
# Example (run explicitly; nothing below executes when sourcing):
# beta.out <- pba.conf.summary.mh.beta(
#   a = 48, b = 378, c = 52, d = 522,
#   p1.alpha = 40, p1.beta = 60,
#   p0.alpha = 60, p0.beta = 40,
#   rr.min = 1.12, rr.mod1 = 1.138, rr.mod2 = 1.162, rr.max = 1.18,
#   effect_measure = "OR", SIMS = 100000
# )
# beta.results <- pba_results(beta.out)
# print(beta.results)
# plot(beta.results, which = "forest")
# plot(beta.results, which = "parameters")
# plot(beta.results, which = "distribution")
# To obtain RR instead, repeat the call with effect_measure = "RR".
