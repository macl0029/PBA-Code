# SCRIPT VERSION: 2026-08-01-v3-individual-level
###############################################################################
...
pba.record.out <- function(data,
  ...
  # Always fit the observed-data model at the individual-record level.
  # When covariates = NULL, this fits outcome ~ exposure rather than replacing
  # the regression with a closed-form 2 x 2-table calculation.
  ...
  valid_predictive <-
    ...
    ppv_exposed >= 0 & ppv_exposed <= 1 & npv_exposed >= 0 & npv_exposed <= 1 &
  ...
  # Two weighted copies of each record are used for the systematic-error-only
  # model: one copy with the true outcome set to 1 and one set to 0. This is an
  # individual-level weighted regression whether or not measured covariates are
  # supplied.
  systematic_data <- bind_rows(
    transform(base, .pba_y = 1L),
    transform(base, .pba_y = 0L)
  )

  for (i in seq_len(n_candidates)) {
    if (isTRUE(progress) && (i %% progress_every == 0L || i == n_candidates)) {
      cat("\rOutcome-misclassification draws: ", i, "/", n_candidates, sep = "")
      flush.console()
    }

    p_systematic <- ifelse(
      x == 1L,
      ifelse(y_obs == 1L, ppv_exposed_expected[i], 1 - npv_exposed_expected[i]),
      ifelse(y_obs == 1L, ppv_unexposed_expected[i], 1 - npv_unexposed_expected[i])
    )
    ...
    p_impute <- ifelse(
      x == 1L,
      ifelse(y_obs == 1L, ppv_exposed[i], 1 - npv_exposed[i]),
      ifelse(y_obs == 1L, ppv_unexposed[i], 1 - npv_unexposed[i])
    )
    if (any(!is.finite(p_impute)) || any(p_impute < 0 | p_impute > 1)) next
    imputed_y <- stats::rbinom(nrow(base), 1L, p_impute)

    # Always fit the completed-data model at the individual-record level.
    # With covariates = NULL, the model is simply outcome ~ exposure.
    imputed_data <- base
    imputed_data$.pba_y <- imputed_y
  ...
  out <- list(
    ...
    model_label = model_label(
      effect_measure, rr_method, prep$covariates
    ),
    exposure = exposure,
    outcome = outcome,
