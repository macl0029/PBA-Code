# SCRIPT VERSION: 2026-08-02-v4-parallel-individual-level
###############################################################################
...
#   - faster individual-level computation using a precomputed model matrix,
#     glm.fit(), warm starts, and direct HC0 covariance calculations
#   - no 2 x 2 analysis shortcut: every simulation imputes every individual
#     and fits the individual-level model, including with continuous covariates
#   - optional chunked multi-core execution using base R PSOCK workers
#   - reproducible per-iteration imputation streams across different core counts
###############################################################################
...
# -----------------------------------------------------------------------------
# Fast individual-level regression helpers
# -----------------------------------------------------------------------------
#
# These helpers do not aggregate the data to a 2 x 2 table. The exposure is
# still imputed separately for every record in every simulation, and every
# completed dataset is analyzed with an individual-level regression. Speed is
# gained by constructing the model matrix once and calling glm.fit() directly.
...
build_effect_matrix <- function(data, covariate_terms = character()) {
  rhs <- c(".pba_x", covariate_terms)
  formula <- stats::reformulate(rhs)
  X <- stats::model.matrix(formula, data = data)
  storage.mode(X) <- "double"

  if (any(!is.finite(X))) {
    stop("The regression model matrix contains non-finite values.",
         call. = FALSE)
  }
...
fit_effect_matrix <- function(X,
  ...
  if (is.null(family)) {
    family <- make_effect_family(effect_measure, rr_method)
  }
  if (is.null(control)) {
  ...
  if (need_se) {
    ...
    # Compute only the column of (X'WX)^(-1) needed for the exposure
    # coefficient. This is exact and avoids constructing a full covariance
    # matrix during every simulation.
    ...
    bread_column <- suppressWarnings(
      try(
        backsolve(
          R,
          forwardsolve(t(R), unit_exposure)
        ),
        silent = TRUE
      )
    )
    ...
    if (effect_measure == "RR" && rr_method == "modified-poisson") {
      # HC0 variance for the exposure coefficient. This is algebraically the
      # exposure diagonal of bread %*% meat %*% bread, computed without
      # creating the full meat or covariance matrices.
      prior_weights <- if (is.null(weights)) rep.int(1, n) else weights
      score_residual <- prior_weights * (y - fit$fitted.values)
      X_rank <- X[, pivot, drop = FALSE]
      score_direction <- drop(X_rank %*% bread_column)
      variance <- sum((score_direction * score_residual)^2)
    } else {
      # Binomial and Poisson dispersion is fixed at one.
...
# Each worker receives one contiguous block of simulations. Within that block,
# fits remain sequential so the previous successful coefficients can still be
# used as warm starts. Every completed dataset remains fully individual-level.
run_exposure_pba_chunk <- function(indices, worker) {
  ...
  systematic_weights <- numeric(2L * worker$n_records)
  systematic_start <- worker$initial_coefficients
  adjusted_start <- worker$initial_coefficients

  fit_fun <- worker$fit_fun
  update_fun <- worker$update_fun
  ...
  for (k in seq_along(indices)) {
    ...
    if (isTRUE(worker$show_progress) &&
      ...
      cat("\rExposure-misclassification draws: ", k, "/", n_local, sep = "")
      flush.console()
    }

    systematic_probabilities <- c(
      worker$ppv_cases_expected[i],
      1 - worker$npv_cases_expected[i],
      worker$ppv_noncases_expected[i],
      1 - worker$npv_noncases_expected[i]
    )
    p_systematic <- systematic_probabilities[worker$cell_id]

    systematic_weights[worker$row_first] <- p_systematic
    systematic_weights[worker$row_second] <- 1 - p_systematic

    systematic_fit <- fit_fun(
    ...
    systematic_start <- update_fun(systematic_fit, systematic_start)
    syst_local[k] <- systematic_fit$effect

    imputation_probabilities <- c(
      worker$ppv_cases[i],
      1 - worker$npv_cases[i],
      worker$ppv_noncases[i],
      1 - worker$npv_noncases[i]
    )
    p_impute <- imputation_probabilities[worker$cell_id]

    if (any(!is.finite(p_impute)) || any(p_impute < 0 | p_impute > 1)) {
      next
    }

    # A separate deterministic seed is assigned to every simulation. Thus a
    # fixed seed produces the same imputations whether cores is 1, 2, or more.
    set.seed(worker$imputation_seeds[i])
    imputed_x <- stats::rbinom(worker$n_records, 1L, p_impute)

    X_work[, worker$exposure_col] <- imputed_x
    imputed_fit <- fit_fun(
      X = X_work,
      y = worker$y,
      effect_measure = worker$effect_measure,
    ...
    adjusted_start <- update_fun(imputed_fit, adjusted_start)
...
pba.record.exp <- function(data,
  ...
  # The observed 2 x 2 table is used only to derive the predictive values for
  # the bias model. It is never used as a substitute for the record-level
  # regression analysis.
  ...
  # Build the record-level design matrix once. Continuous covariates, factor
  # covariates, and any combination of the two are retained at the individual
  # level throughout the analysis.
  design <- build_effect_matrix(base, prep$covariate_terms)
  X_base <- design$X
  exposure_col <- design$exposure_col
  y <- base$.pba_y
  x_obs <- base$.pba_x
  ...
  if (is.null(observed_fit)) {
    stop("The observed-data individual-level regression did not fit.",
         call. = FALSE)
  }
  ...
  eff_random_only <- add_random_error(
    rep(observed_effect, niter),
    rep(observed_se, niter),
    effect_measure
  )
  ...
  # Predictive values based on the expected corrected table are used for the
  # systematic-error-only regression. A beta draw around the corrected
  # prevalence is used for each stochastic individual-level reclassification.
  ...
  ppv_cases_expected <-
    ...
    (se_cases * prev_cases_expected +
       (1 - sp_cases) * (1 - prev_cases_expected))
  npv_cases_expected <-
    sp_cases * (1 - prev_cases_expected) /
    ((1 - se_cases) * prev_cases_expected +
       sp_cases * (1 - prev_cases_expected))
  ...
  ppv_noncases <-
    ...
    (se_noncases * prev_noncases +
       (1 - sp_noncases) * (1 - prev_noncases))
  npv_noncases <-
    sp_noncases * (1 - prev_noncases) /
    ((1 - se_noncases) * prev_noncases +
       sp_noncases * (1 - prev_noncases))

  valid_predictive <-
    is.finite(ppv_cases) &
    is.finite(npv_cases) &
    is.finite(ppv_noncases) &
    is.finite(npv_noncases) &
    is.finite(ppv_cases_expected) &
    is.finite(npv_cases_expected) &
    is.finite(ppv_noncases_expected) &
    is.finite(npv_noncases_expected) &
  ...
  if (n_candidates == 0L) {
    stop("No valid predictive values were produced.", call. = FALSE)
  }

  # Four observed exposure-outcome categories determine the reclassification
  # probabilities. The identities and continuous covariate values of all
  # records are nevertheless retained in every regression.
  cell_id <- ifelse(
    y == 1L,
    ifelse(x_obs == 1L, 1L, 2L),
    ifelse(x_obs == 1L, 3L, 4L)
  )

  # Precompute the two record-level copies used for the systematic-error fit.
  # This is exactly the same weighted individual-level regression used in the
  # slower code; only the repeated model-frame construction has been removed.
  X_systematic_1 <- X_base
  X_systematic_0 <- X_base
  X_systematic_1[, exposure_col] <- 1
  X_systematic_0[, exposure_col] <- 0
  X_systematic <- rbind(X_systematic_1, X_systematic_0)
  y_systematic <- c(y, y)

  row_first <- seq_len(n_records)
  row_second <- n_records + row_first

  # Generate one seed and one total-error standard-normal draw per candidate
  # on the master process. This makes results reproducible across core counts.
  ...
  worker_data <- list(
    ...
    cell_id = cell_id,
    ppv_cases_expected = ppv_cases_expected,
    npv_cases_expected = npv_cases_expected,
    ppv_noncases_expected = ppv_noncases_expected,
    npv_noncases_expected = npv_noncases_expected,
    ppv_cases = ppv_cases,
    npv_cases = npv_cases,
    ppv_noncases = ppv_noncases,
    npv_noncases = npv_noncases,
    imputation_seeds = imputation_seeds,
    initial_coefficients = observed_fit$coefficients,
    effect_measure = effect_measure,
    rr_method = rr_method,
    family = family,
    control = control,
  ...
  if (cores_used == 1L) {
    chunk_results <- list(
      run_exposure_pba_chunk(chunks[[1L]], worker_data)
    )
    parallel_backend <- "sequential"
  } else {
    if (isTRUE(progress)) {
      cat(
        "Exposure-misclassification draws: ",
        scales::comma(n_candidates),
        " simulations on ", cores_used, " PSOCK workers...\n",
        sep = ""
      )
    }
    ...
    if (isTRUE(progress)) {
      cat("Parallel simulation blocks completed.\n")
    }
  }
  ...
  for (chunk_result in chunk_results) {
    ...
    syst[idx] <- chunk_result$syst
    adjusted[idx] <- chunk_result$adjusted
    adjusted_se[idx] <- chunk_result$adjusted_se
    accepted[idx] <- chunk_result$accepted
  }

  rejected_model <- sum(!accepted)
  if (!any(accepted)) {
    stop("No valid individual-level analyses were produced.", call. = FALSE)
  }

  accepted_candidate_index <- which(accepted)
  syst <- syst[accepted]
  adjusted <- adjusted[accepted]
  adjusted_se <- adjusted_se[accepted]
  total_z_accepted <- total_error_z[accepted]

  eff_total <- if (effect_measure %in% c("OR", "RR")) {
    exp(log(adjusted) + total_z_accepted * adjusted_se)
  } else {
    adjusted + total_z_accepted * adjusted_se
  }
  ...
  accepted_index <- accepted_candidate_index[valid_total]
  ...
  out <- list(
    ...
    rejection_counts = tibble(
      Stage = c(
        "Incompatible corrected table",
        "Invalid predictive value",
        "Failed/invalid individual-level analysis",
    ...
    computation_path = paste0(
      "individual-level imputation; precomputed model matrix with glm.fit(); ",
      cores_used,
...
# Example
# -----------------------------------------------------------------------------
# Set RUN_EXAMPLE to TRUE to run. The example contains a continuous covariate;
# the program does not collapse completed datasets to 2 x 2 tables.
...
if (RUN_EXAMPLE) {
  ...
  # cores = 1L runs sequentially. Increase cores for chunked PSOCK parallelism.
  # A fixed seed gives reproducible imputations across different core counts.
